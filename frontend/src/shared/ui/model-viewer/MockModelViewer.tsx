...
function rotatePoint([x, y, z]: [number, number, number], rotation: RotationState) {
...
function parseObj(text: string): MeshData | null {
  ...
  const centeredVertices = vertices.map(([x, y, z]) => [x - center[0], y - center[1], z - center[2]] as [number, number, number]);
  const radius = Math.max(
    ...centeredVertices.map(([x, y, z]) => Math.sqrt(x * x + y * y + z * z)),
    1
  );

  return {
    vertices: centeredVertices.map(([x, y, z]) => [x / radius, y / radius, z / radius]),
    edges: Array.from(edges).map((edge) => edge.split("-").map(Number) as [number, number]),
  };
}

export function MockModelViewer({ fileName, modelUrl, mode = "card", onExpand }: MockModelViewerProps) {
  ...
  useEffect(() => {
    ...
    function draw() {
      ...
      context.lineWidth = isModal ? 1.8 : 1.4;
      context.strokeStyle = "rgba(32, 32, 32, 0.72)";
  ...
  function handleWheel(event: WheelEvent<HTMLDivElement>) {
    ...
    setRotation((current) => ({
      ...current,
      scale: Math.min(2.4, Math.max(0.55, current.scale - event.deltaY * 0.0012)),
    }));
  }
  ...
  return (
    <div className={isModal ? styles.viewerModal : styles.viewerCard}>
      ...
      >
        ...
        <div className={styles.fileBadge} title={fileName}>
          {truncateFileName(fileName, isModal ? 48 : 30)}
        </div>
      ...
      <div className={styles.controls}>
        <button type="button" className={styles.controlButton} onClick={() => updateZoom(rotation.scale + 0.15)}>
          +
        </button>
        <button type="button" className={styles.controlButton} onClick={() => updateZoom(rotation.scale - 0.15)}>
          -
        </button>
        <button type="button" className={styles.controlButton} onClick={() => rotateZ(-12)}>
          Z-
        </button>
        <button type="button" className={styles.controlButton} onClick={() => rotateZ(12)}>
          Z+
        </button>
        <button type="button" className={styles.controlButtonWide} onClick={resetView}>
          Сбросить вид
        </button>
      </div>

      <p className={styles.hint}>Потяните сцену мышью для вращения по X/Y, колесо отвечает за масштаб, кнопки Z меняют угол по оси Z.</p>
    </div>
  );
}
