...
def configure_cycles_denoising():
    cycles = bpy.context.scene.cycles
    denoiser_property = cycles.bl_rna.properties.get("denoiser")
    available_denoisers = []
    if denoiser_property is not None and hasattr(denoiser_property, "enum_items"):
        available_denoisers = [
            item.identifier
            for item in denoiser_property.enum_items
        ]

    preferred_denoisers = ["OPENIMAGEDENOISE", "OPTIX", "NLM"]
    selected_denoiser = next(
        (denoiser for denoiser in preferred_denoisers if denoiser in available_denoisers),
        None,
    )

    cycles.use_denoising = selected_denoiser is not None
    if selected_denoiser is not None:
        cycles.denoiser = selected_denoiser
